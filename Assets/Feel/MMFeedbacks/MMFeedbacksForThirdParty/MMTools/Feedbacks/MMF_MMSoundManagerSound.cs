	...
	public class MMF_MMSoundManagerSound : MMF_Feedback
	{
		...
		protected virtual void PlaySound(AudioClip sfx, Vector3 position, float intensity)
		{
			if (DoNotPlayIfClipAlreadyPlaying) 
			{
				...
				AudioSource foundSource = MMSoundManager.Instance.FindByClip(sfx);
				//if(MMSoundManager.Instance.FindByClip(sfx) != null) //OG Line Of Code.
				if (foundSource != null && foundSource.isPlaying)
				{
					return;
				}
			}
